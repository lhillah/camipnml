 ...
 */
public final class CoreModel2Cami {
	/**
	...
	private void buildNet(CamiRepository cr, final PetriNetHLAPI netElement) throws CamiException {
		...
		if (netElement != null) {
			try {
				String netName = netElement.getNameHLAPI().getText();
				if (netName == null) {
					netName = "defaultCamiNetName";
				}
				if (cr == null) {
					JOURNAL.info("PTNet2CamiModel#buildNetNode:" + "CamiRepository argument was null...Creating a Cami repository.");
			...
			throw new CamiException("PTNet2CamiModel#buildNetNode: the PNML net is null.");
		}
	}
	...
	private void buildTransition(final CamiRepository cr, final TransitionHLAPI transition) throws CamiException {
		try {
			...
			if (transition.getNodegraphicsHLAPI() != null && transition.getNodegraphicsHLAPI().getPositionHLAPI() != null) {
				final Po placePosition = MF.createPo();
				placePosition.setPo(transitionID, transition.getNodegraphicsHLAPI().getPositionHLAPI().getX(), transition.getNodegraphicsHLAPI()
