...
import fr.lip6.move.pnml.symmetricnet.hlcorestructure.hlapi.AnnotationGraphicsHLAPI;
import fr.lip6.move.pnml.symmetricnet.hlcorestructure.hlapi.ArcGraphicsHLAPI;
import fr.lip6.move.pnml.symmetricnet.hlcorestructure.hlapi.ArcHLAPI;
import fr.lip6.move.pnml.symmetricnet.hlcorestructure.hlapi.NameHLAPI;
import fr.lip6.move.pnml.symmetricnet.hlcorestructure.hlapi.NodeGraphicsHLAPI;
import fr.lip6.move.pnml.symmetricnet.hlcorestructure.hlapi.NodeHLAPI;
import fr.lip6.move.pnml.symmetricnet.hlcorestructure.hlapi.OffsetHLAPI;
import fr.lip6.move.pnml.symmetricnet.hlcorestructure.hlapi.PNTypeHLAPI;
...
import fr.lip6.move.pnml.symmetricnet.hlcorestructure.hlapi.PositionHLAPI;
import fr.lip6.move.pnml.symmetricnet.hlcorestructure.hlapi.TransitionHLAPI;

/**
 * Converts PN models from Cami into PNML Symmetric Net Graphs (SN).
 * 
 * @author lom
 * 
 */
public final class Cami2SN {
	...
	/**
	 * Returns the constructed SN Petri net.
	 * 
	 * @return PetriNetHLAPI the constructed SN net.
	 */
	 ...
	 *            Cami repository
	 */
	private void setPlaceName(final PlaceHLAPI placeNode, final Cn pl,
			final Po placePosition, final CamiRepository cr) {
				...
				if (aPt.getNumObject() == pl.getNodeId()) {
					if ("name".equalsIgnoreCase(aPt.getAttributeName())) {
						if (placePosition != null) {
							offX = Integer.valueOf(placePosition.getH()
									- aPt.getH());
							offY = Integer.valueOf(placePosition.getV()
									- aPt.getV());
							if (offX != null && offY != null) {
								final OffsetHLAPI myOffset = new OffsetHLAPI(
										offX, offY);
								final AnnotationGraphicsHLAPI myAnnotation = new AnnotationGraphicsHLAPI(
										myOffset, null, null, null);
								nodeNameHLAPI
										.setAnnotationgraphicsHLAPI(myAnnotation);
							}
						}
						break;
					}
	 ...
	 *            Cami repository.
	 */
	private void setPlaceMarking(final PlaceHLAPI placeNode, final Cn pl,
			final Po nodePosition, final CamiRepository cr) {
		// TODO implement this
		...
		try {
			if (allAr != null) {
				for (final Ca aAr : allAr) {
					final String arcId = myRunner.transformCamiId2XmlId(aAr
							.getArcID());
					cr.relateCamiPnmlIds(aAr.getArcID(), arcId);
					// get nodes source and target Ids
					final String sId = cr.getPnmlIdOfCamiId(aAr
							.getStartNodeID());
					final String tId = cr.getPnmlIdOfCamiId(aAr.getEndNodeID());
					final IdRepository pnmlIdRep = pnmlMr
							.getCurrentIdRepository();
					// creates the arc
					final ArcHLAPI myArc = new ArcHLAPI(arcId,
	 ...
	 *            Cami repository
	 */
	private void setArcPositions(final ArcHLAPI arc, final Ca ar,
			final CamiRepository cr) {
		...
		if (arcPositions != null) {
			...
			for (final Pi aPi : arcPositions) {
				final PositionHLAPI myPosition = new PositionHLAPI(Integer
						.valueOf(aPi.getH()), Integer.valueOf(aPi.getV()));
	 ...
	 *             CamiException
	 */
	private void setArcInscription(final ArcHLAPI arc, final Ca ar,
			final CamiRepository cr) throws CamiException {
		// TODO: implement this
		System.out.println("Not yet implemented.");
	}
	 ...
	 *             CamiException
	 */
	public final void setTransitions(final CamiRepository cr)
			throws CamiException {
		...
		try {
			if (allTr != null) {
				for (final Cn aTr : allTr) {
					...
					final PositionHLAPI myPosition = new PositionHLAPI(Integer
							.valueOf(nodePosition.getH()), Integer
							.valueOf(nodePosition.getV()));
	 ...
	 *            Cami repository
	 */
	private void setTransitionName(final TransitionHLAPI transitionNode,
			final Cn tr, final Po nodePosition, final CamiRepository cr) {
			...
			if (aPt.getNumObject() == tr.getNodeId()) {
				if ("name".equalsIgnoreCase(aPt.getAttributeName())) {
					if (nodePosition != null) {
						offX = Integer
								.valueOf(nodePosition.getH() - aPt.getH());
						offY = Integer
								.valueOf(nodePosition.getV() - aPt.getV());
						if (offX != null && offY != null) {
							final OffsetHLAPI myOffset = new OffsetHLAPI(offX,
									offY);
							final AnnotationGraphicsHLAPI myAnnotation = new AnnotationGraphicsHLAPI(
									myOffset, null, null, null);
							nodeNameHLAPI
	 ...
	 *            Cami repository.
	 */
	private void setTransitionGuard(final TransitionHLAPI trNode, final Cn tr,
			final Po nodePosition, final CamiRepository cr) {
		// TODO implement this
		System.out.println("Not Yet Implemented.");
	}
