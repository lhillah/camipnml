...
import fr.lip6.move.pnml.pthlpng.hlcorestructure.hlapi.AnnotationGraphicsHLAPI;
import fr.lip6.move.pnml.pthlpng.hlcorestructure.hlapi.ArcGraphicsHLAPI;
import fr.lip6.move.pnml.pthlpng.hlcorestructure.hlapi.ArcHLAPI;
import fr.lip6.move.pnml.pthlpng.hlcorestructure.hlapi.NameHLAPI;
import fr.lip6.move.pnml.pthlpng.hlcorestructure.hlapi.NodeGraphicsHLAPI;
import fr.lip6.move.pnml.pthlpng.hlcorestructure.hlapi.NodeHLAPI;
import fr.lip6.move.pnml.pthlpng.hlcorestructure.hlapi.OffsetHLAPI;
import fr.lip6.move.pnml.pthlpng.hlcorestructure.hlapi.PNTypeHLAPI;
...
import fr.lip6.move.pnml.pthlpng.hlcorestructure.hlapi.PositionHLAPI;
import fr.lip6.move.pnml.pthlpng.hlcorestructure.hlapi.TransitionHLAPI;

/**
 * Converts Petri net models from Cami into PNML.
 * 
 * @author lom
 * 
 */
public final class Cami2PTHLPNG {
	/**
	...
	/**
	 * Returns the constructed PTHLPNG Petri net.
	 * 
	 * @return PetriNetHLAPI the constructed PTHLPNG net.
	 */
		...
		try {
			if (allPl != null) {
				...
				for (final Cn aPl : allPl) {
					...
					// TODO : check addObjectHLPAI(...) in PFW
					defPage.getObjects_hlcorestructure_PlaceHLAPI().add(placeNode);
				}
			}
		} catch (final InvalidIDException iie) {
			throw new CamiException(iie.getMessage());
		...
		try {
			if (allAr != null) {
				for (final Ca aAr : allAr) {
					...
					// respect.
					defPage.getObjects_hlcorestructure_ArcHLAPI().add(myArc);
				}
			}
	...
	private void setArcPositions(final ArcHLAPI arc, final Ca ar, final CamiRepository cr) {
		...
		if (arcPositions != null) {
			...
			for (final Pi aPi : arcPositions) {
				...
				myArcGraphics.getPositionsHLAPI().add(myPosition);
			}
		}
	}
		...
		try {
			if (allTr != null) {
				for (final Cn aTr : allTr) {
					...
					defPage.getObjects_hlcorestructure_TransitionHLAPI().add(transitionNode);
				}
			}
		} catch (final InvalidIDException iee) {
			throw new CamiException(iee.getMessage());
	...
	private void setTransitionGuard(final TransitionHLAPI trNode, final Cn tr, final Po nodePosition, final CamiRepository cr) {
		// TODO implement this
		System.out.println(this.getClass().getCanonicalName() + ": transiion condition not yet implemented.");
	}
